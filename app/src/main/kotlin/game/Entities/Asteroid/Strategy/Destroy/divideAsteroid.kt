...
class divideAsteroid(private val size : Int) : DestroyStrategy<Asteroid> {
    ...
    override fun destroy(asteroid: Asteroid): List<Asteroid> {
        ...
        for (i in 0 until size) {
            asteroids.add(Asteroid(asteroid.getHealth()/ size , asteroid.getSize()/ size, asteroid.getPoints(), destroyAsteroid(), straightMovement(0.0, asteroid.getVelocity(), asteroid.getPosition(), Position(asteroid.getPosition().getX() + 10* i , asteroid.getVelocityY() + 2 * i)), asteroid.getPosition(), asteroid.getVelocityX(), asteroid.getVelocityY(), asteroid.getId(),asteroid.getObserverss() ,asteroid.getObservers()))
        }
        return asteroids
    }
}
