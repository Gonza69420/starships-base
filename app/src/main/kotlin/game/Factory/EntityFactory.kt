...
class EntityFactory {
    ...
    fun createShip(difficulty : String) : Ship {
        if (difficulty.equals("easy")){
            return Ship(normalGun(1, 100, 1), SHIP_HEALTH_EASY, Position(500.0, 500.0), 0.0, 0.0, 0)
        } else if (difficulty.equals("medium")){
            return Ship(normalGun(1, 100, 1), SHIP_HEALTH_Medium, Position(500.0, 500.0), 0.0, 0.0, 0)
        } else if (difficulty.equals("hard")){
            return Ship(normalGun(1, 100, 1), SHIP_HEALTH_Hard, Position(500.0, 500.0), 0.0, 0.0, 0)
        }
        return Ship(normalGun(1, 100, 1), SHIP_HEALTH_Medium, Position(500.0, 500.0), 0.0, 0.0, 0)
    }
    ...
    fun  createAsteroid(id : Int, random : Double, random2 : Double, random3 : Double, random4 : Double) : Asteroid{
        ...
        val finalPositionLeftRight = Position(WIDTH, (HEIGHT - 0.1 * random3) + 0.1)
        if (random < 0.2 ) {
            if (random < 0.05) {
